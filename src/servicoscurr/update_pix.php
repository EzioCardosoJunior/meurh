...
// === VERIFICAR SE EXISTE CADASTRO PIX ===
$stmt = $pdo->prepare("SELECT id FROM pix WHERE id_usuario = :id_usuario LIMIT 1");
$stmt->execute([':id_usuario' => $id_usuario]);

if ($stmt->rowCount() === 0) {
    http_response_code(404);
    echo json_encode(['erro' => 'Nenhum PIX encontrado para este usuário.']);
    exit;
}
...
if ($ok) {
    ...
    http_response_code(500);
    echo json_encode(['erro' => 'Falha ao atualizar chave PIX.']);
}
