...
if (!$data || empty($data['id_usuario']) || empty($data['empresa']) || empty($data['funcao']) || empty($data['data_entrada'])) {
...
// Zerar data de término se for emprego atual
if ($atual == 1) {
    $data_termino = null;
}

$sql = "INSERT INTO emprego_atual
        (id_usuario, empresa, funcao, data_entrada, data_termino, atual, atividades)
        VALUES
        (:id_usuario, :empresa, :funcao, :data_entrada, :data_termino, :atual, :atividades)";
